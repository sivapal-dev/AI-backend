...
class AIClient:
    ...
    def __init__(self):
        ...
        self.default_model = settings.openrouter_default_model or OPENROUTER_MODELS[0]
    ...
    def _pick_model(self, model: Optional[str] = None) -> str:
        """Pick model: explicit > default > random from pool."""
    ...
    async def _call_openrouter(
        ...
        payload = {
            ...
            # Some OpenRouter models don't support response_format, so we omit it
            # and rely on system prompt + fallback parsing
        }
    ...
    async def chat_completion(
        ...
        if chosen_provider in ("auto", "openrouter"):
            ...
            if self.openrouter_key and self.openrouter_key.startswith("sk-"):
                try:
                    chosen_model = self._pick_model(model)
        ...
        if chosen_provider in ("auto", "openrouter"):
            if self.openrouter_key and self.openrouter_key.startswith("sk-"):
                try:
                    ...
                    if chosen_provider == "openrouter":
                        raise AIClientError(f"OpenRouter streaming failed: {e}")
                    # auto mode: try Mistral

        # Fallback: use non-streaming Mistral and yield full response at once
        if chosen_provider in ("auto", "mistral"):
            if self.mistral_key and self.mistral_key.strip():
                full = await self.chat_completion(
                    system_prompt=system_prompt,
